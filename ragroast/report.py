...
def render(results: Dict[str, Result], dataset: str, n_docs: int, n_queries: int, k: int, params: str) -> str:
    ...
    lines: List[str] = [
        "",
        f"  ragroast · retrieval showdown on '{dataset}'  ({n_docs} docs · {n_queries} queries · k={k})",
        "",
        header,
        rule,
    ]
    for name, r in results.items():
        ...
        lines.append(
            f"  {name:<20}{cell}{r.recall:>12.3f}{r.mrr:>8.3f}{r.latency_ms:>10.2f} ms"
        )
    lines.append(rule)
